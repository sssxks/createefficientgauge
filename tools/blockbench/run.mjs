...
export function parseArgs(argv) {
    ...
    for (let index = 0; index < argv.length; index += 1) {
        ...
        if (argument === "--out" || argument === "--blockbench" || argument === "--timeout") {
...
export function blockbenchCandidates(explicitPath, environment = process.env) {
    ...
    if (process.platform === "win32") {
        ...
        add(path.join(os.homedir(), "Applications", "Blockbench.app", "Contents", "MacOS", "Blockbench"));
    } else {
...
export function findBlockbenchExecutable(explicitPath, environment = process.env) {
    ...
    if (requested) {
        throw new Error(`Blockbench executable does not exist: ${path.resolve(requested)}`);
    }
    throw new Error(
...
async function waitForRenderer(child, port, deadline, getLaunchError = () => undefined) {
    ...
    while (Date.now() < deadline) {
        ...
        if (child.exitCode !== null) {
            throw new Error(`Blockbench exited before its renderer was ready (exit code ${child.exitCode})`);
        }
        try {
    ...
    const detail = lastError?.message ? ` Last connection error: ${lastError.message}` : "";
...
export class CdpClient {
    constructor(url, timeoutMs) {
        if (typeof WebSocket === "undefined") {
            throw new Error("this runner requires a Node.js release with the WebSocket global");
        }
        this.nextId = 1;
    ...
    async connect() {
        ...
        await new Promise((resolve, reject) => {
            const timer = setTimeout(() => {
                cleanup();
                reject(new Error("timed out connecting to Blockbench's debugging endpoint"));
            }, this.timeoutMs);
            const onOpen = () => {
                cleanup();
                resolve();
            ...
            const onError = () => {
                cleanup();
                reject(new Error("failed to connect to Blockbench's debugging endpoint"));
            };
            const onClose = () => {
                cleanup();
                reject(new Error("Blockbench closed its debugging endpoint before connection"));
            };
            const cleanup = () => {
                clearTimeout(timer);
    ...
    send(method, params = {}, timeoutMs = this.timeoutMs) {
        if (this.webSocket.readyState !== WebSocket.OPEN) {
            return Promise.reject(new Error("the Blockbench debugging connection is not open"));
        }
    ...
    return (async () => {
        ...
        const output = {
            ...
            model(relativePath, contents = Codecs.java_block.compile()) {
                const serialized = typeof contents === "string"
                    ? contents
                    : `${JSON.stringify(contents, null, 2)}\n`;
                return recordArtifact(relativePath, serialized, "utf8", "model");
            },

            async preview(relativePath, options = { crop: true, height: 512, width: 512 }) {
                if (!Preview.selected) {
            ...
            texture(relativePath, texture = Texture.getDefault()) {
                ...
                return recordArtifact(relativePath, texture.getDataURL(), "data-url", "texture");
            },
        };
        ...
        if (typeof build !== "function") {
            throw new TypeError("the Blockbench job must export a function with module.exports");
        }
...
async function waitForBlockbenchApi(client, deadline) {
    ...
    while (Date.now() < deadline) {
        try {
            if (await evaluate(client, readinessExpression, Math.min(2_000, remainingTime(deadline)))) {
                return;
            }
        } catch {
...
function observeBlockbenchConsole(client) {
    return client.onEvent((message) => {
        if (message.method === "Runtime.consoleAPICalled") {
            ...
            const writer = type === "error" || type === "warning" ? console.error : console.log;
...
async function stopBlockbench(child, client) {
    ...
    if (client) {
        try {
            ...
            // The process may close the debugging socket before returning.
        }
    }
...
async function runJob(options) {
    ...
    const profileDirectory = await mkdtemp(path.join(os.tmpdir(), "blockbench-batch-"));
    ...
    try {
        ...
        child.stdout?.on("data", (chunk) => process.stdout.write(`blockbench: ${chunk}`));
        child.stderr?.on("data", (chunk) => process.stderr.write(`blockbench: ${chunk}`));

        const target = await waitForRenderer(child, port, deadline, () => launchError);
        client = new CdpClient(target.webSocketDebuggerUrl, remainingTime(deadline));
        await client.connect();
        await client.send("Runtime.enable", {}, Math.min(5_000, remainingTime(deadline)));
        ...
        const result = await evaluate(
            client,
            buildJobExpression(source, options.jobPath, options.outDir, options.jobArgs),
        ...
        console.log(`Blockbench ${result.blockbenchVersion}; ${result.artifacts.length} artifact(s); ${result.durationMs} ms`);
        for (const artifact of result.artifacts) {
            console.log(`  ${artifact.path} (${artifact.kind}, ${artifact.bytes} bytes)`);
        }
